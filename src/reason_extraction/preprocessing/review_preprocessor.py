...
#------------------
            # 音＋外す＝動詞
            elif len(w_lemmas)>0 and features[0]=='動詞' and features[6] in ['外す','外れる','ずれる']\
                    and len(w_lemmas)!=0 and w_poses[-1]=='名詞' and w_lemmas[-1] in ['音','音程']:
            ...
            # 音＋助詞＋外す＝動詞
            elif len(w_lemmas)>1 and features[0]=='動詞' and features[6] in ['外す','外れる','ずれる']\
                    and len(w_lemmas)!=0 and w_poses[-1]=='助詞'\
                        and w_poses[-2]=='名詞' and w_lemmas[-2] in ['音','音程']:
                        
                # 直前の原形
                ## 助詞は除外する
                w_lemmas[-2]=w_surfaces[-2]+features[6]
                ...
                w_starts.pop()
                w_ends.pop()
            # (頭/調子/マナー/モチベーション/気持ち/場所)悪い
            elif len(w_lemmas)>0 and features[0]=='形容詞' and features[6] == '悪い'\
                    and len(w_lemmas)!=0 and w_poses[-1]=='名詞' and w_lemmas[-1] in ['頭','調子','マナー','モチベーション','気持ち','場所']:
                        
                # 直前の原形 ← 直前の表層形+['悪い']に変更 e.g.'頭悪い'
                w_lemmas[-1]=w_surfaces[-1]+features[6]
                ...
                w_sub_poses[-1]='自立'
                w_ends[-1]=w_ends[-1]+len(node.surface)
            # (頭/調子/マナー/モチベーション/気持ち/場所)+助詞+悪い                
            elif len(w_lemmas)>1 and features[0]=='形容詞' and features[6] == '悪い'\
                    and len(w_lemmas)!=0 and w_poses[-1]=='助詞'\
                        and w_poses[-2]=='名詞' and w_lemmas[-2] in ['頭','調子','マナー','モチベーション','気持ち','場所']:
                ...
                w_ends.pop()
            # 持ち歌 (mecab辞書登録後削除)
            elif len(w_lemmas)>0 and features[0]=='名詞' and features[6] in ['歌','曲']\
                    and len(w_lemmas)!=0 and w_poses[-1]=='動詞' and w_lemmas[-1] =='持つ' and w_surfaces[-1]=='持ち':
            ...
            # 上記助動詞を除いた全品詞の単語情報を格納
                ...
                # 以下mecab辞書変更するまでの一時的処理---

                if node.surface in ['めっさ','ガチ','ガチで','本当','まじ','まじで','マジ','マジで','ほんま','ほんと','ホント','ほぼほぼ','超絶']:
                    features[0]='副詞'
                elif node.surface in ['笑','泣','爆笑','涙','殴','死'] and features[0]=='名詞':
                    features[0]='動詞' # 文末の笑等は述語扱い(N単語としてヒットさせるため原形をN辞書に追加必要)                    
                elif node.surface in ['きもい']:
                    features[0]='形容詞'
                elif features[1]=='形容動詞語幹':
                    features[0]='形容動詞' # mecabでは品詞：形容動詞がないためリストで定義
                elif features[6] in ['地獄','音痴','最悪','最高','心配','不安','無能','絶望','絶望的','くそ','クソ','ショック','めちゃめちゃ','めちゃくちゃ']:
                    features[0]='形容動詞'                    
                    features[1]='形容動詞語幹'
                elif features[6]=='出し手':
                    features[6]='出る'
                    ...
                    features[1]='副詞可能'    
                elif node.surface in ['w','ww','www'] and features[0]=='名詞': # 文末w
                    features[6]='|'
                    features[0]='記号'
